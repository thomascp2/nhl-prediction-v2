...
try:
    ...
    print(f"âœ… PASS: Database connected")
...
try:
    ...
    row = cursor.fetchone()
    if not row:
        print("âš ï¸ WARNING: No test data for 2025-10-15, using generic test")
    ...
    if missing:
        print(f"âŒ FAIL: Missing features: {missing}")
        extractor.close()
        sys.exit(1)
    
    print(f"âœ… PASS: Binary features extracted")
    ...
    print(f"âŒ FAIL: Binary extractor error - {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
try:
    ...
    if row:
        ...
        print(f"âœ… PASS: Continuous features extracted")
        print(f"    Player: {player}")
        print(f"    Features: {len(features)}")
        ...
        print("âš ï¸ WARNING: No test data available, skipping")
    
    extractor.close()
    
except Exception as e:
    print(f"âŒ FAIL: Continuous extractor error - {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
try:
    ...
    row = cursor.fetchone()
    if not row:
        print("âš ï¸ WARNING: No players with 5+ games, skipping")
        conn.close()
    else:
        player, team = row
        conn.close()
        ...
        if pred is None:
            print("    âš ï¸ Insufficient data (expected for some players)")
        else:
            ...
            if has_features:
                print(f"    âœ… Correct feature names")
            else:
                print(f"    âŒ Missing expected features")
    ...
    print()
    print("âœ… PASS: Statistical prediction engine working")
    
except Exception as e:
    print(f"âŒ FAIL: Prediction engine error - {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
try:
    ...
    if 'features_json' not in columns:
        print("âŒ FAIL: features_json column missing from predictions table")
        conn.close()
        sys.exit(1)
    ...
    if count_with_features > 0:
        ...
        print(f"âœ… PASS: Features being saved to database")
        ...
        print("âš ï¸ WARNING: No predictions with features yet")
    ...
    print(f"âŒ FAIL: Database check error - {e}")
    sys.exit(1)
